    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <ModeButton
          ...
          label="Pencere"
          description="Tek uygulama paylaş"
      ...
      {mode === "fullscreen" && monitors.length > 1 && (
        <div className="flex flex-col gap-1.5">
          <label className="text-xs text-[var(--text-muted)]">Ekran</label>
          <select
            id="select-monitor"
          ...
          >
            {monitors.map((m) => (
              <option key={m.index} value={m.index}>
                {m.name} {m.isPrimary ? "(Birincil)" : ""} — {m.width}×{m.height}
              </option>
            ))}
          </select>
        </div>
      ...
      {mode === "window" && (
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            <label className="text-xs text-[var(--text-muted)]">Uygulama Penceresi</label>
            <button
          ...
          {windows.length === 0 ? (
            <div className="px-3 py-4 text-center text-xs text-[var(--text-muted)] bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)]">
              {windowsLoading ? "Pencereler yükleniyor..." : "Açık pencere bulunamadı"}
            </div>
          ) : (
