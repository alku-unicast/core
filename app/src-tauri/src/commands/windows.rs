...
pub async fn get_open_windows() -> Result<Vec<WindowInfo>, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        unsafe {
            ...
            let array_ref =
                CGWindowListCopyWindowInfo(options, core_graphics::window::kCGNullWindowID);
            ...
            if !array_ref.is_null() {
                ...
                for i in 0..array.len() {
                    ...
                    if let (Some(id_p), Some(owner_p)) = (id_raw, owner_raw) {
                        ...
                        let owner_str: CFString =
                            TCFType::wrap_under_get_rule(*owner_p as *const _);

                        let title = if let Some(t_p) = title_raw {
                            let t_str: CFString =
                                TCFType::wrap_under_get_rule(*t_p as *const _);
                            t_str.to_string()
                        } else {
                            String::new()
                        };
...
fn enum_windows_win32() -> Result<Vec<WindowInfo>, String> {
    ...
    unsafe {
        EnumWindows(Some(enum_proc), LPARAM(&results_ptr as *const _ as isize))
            .map_err(|e| e.to_string())?;
    }
